...
*   **`run_auto_stop_loss.py` (米国株)**
    *   **役割**: moomoo（Futu OpenD API）を介して、保有する米国株ロングポジションの逆指値注文（STOP）を自動的に計算・更新します。
...
*   **`stop_order_manager.py`**
    *   moomoo用の逆指値注文の発注・更新処理をカプセル化したモジュールです。`STOP` 注文タイプを使用し、通常取引時間（`session=Session.RTH`）および特定口座（`jp_acc_type=SubAccType.JP_TOKUTEI`）向けに発注します。
*   **`kabu_client.py`**
